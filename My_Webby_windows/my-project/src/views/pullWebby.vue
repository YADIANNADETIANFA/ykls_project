    ...
    <div class="pullListNew">
      ...
      <el-timeline style="width: 95%">
        ...
        <el-pagination class="mpage"
                       ...
                       :total="this.total"
                       @current-change=this.getPullListPageRes
                       v-if="this.total > 0">
        </el-pagination>
      </el-timeline>
    </div>
...
export default {
  ...
  methods: {
    getPullListPageRes(currentPage) {
      const _this = this;
      $.ajax.get(
        'pullfeedsWebby',
        {
        ...
        if (res.data.code === "my200") {
          ...
          _this.$router.push({path:'/loginWebby', query:{next: "/pullfeeds"}});
        }
      }).catch(error => {
        console.log(error);
        //服务器异常，请联系管理员
